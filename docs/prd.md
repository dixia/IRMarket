...
| Demo 叙事 | 沿用"溜溜梅（LLM）"虚构叙事作 UI 演示文案（公开资料 06658 对应"锅圈食品"，与叙事不符则仅改文案，不影响机制） |
...
2. **手续费（❓ Q7 待拍板）**：〔v0.8 R14〕Q3 曾选"A 显式扣费"，但用户钱包直接操作 Monoracle 时**合约内无扣费位**；且费用**不能**嵌入报价价差（任何系统性偏离公平价都会被否决套利瞬间吃掉）。可行方案见 §十 Q7：
   - **方案 1（推荐，若开手续费）**：交易经 IRMarket 包裹层入口，进 veto 前按名义额显式扣 1% 至做市商地址；
   - **方案 2（demo 最简）**：Demo 无手续费，bot 在演示期充当纯对手方。
3. **收益归属**：手续费（若有）归 bot/做市商；bot 的对赌盈亏 = 用户盈亏的镜像
4. **用户感知**：UI 明确展示"你将按报价 P 与做市商换手"及手续费（若有），不做隐性价差
...
- **标的详情页（核心交易页）**：
  ...
  - 右卡（交易面板）：
    ...
    - 预览：`将按 P 价格与做市商换手：付 X HKD / 收 Y LLM（或反之）` + 最大亏损提示（= 投入本金）
...
| 模块 | Demo 包含 | 暂不实现 |
|------|-----------|----------|
| 标的 | 真实代码 06658.HK（HKG:6658），bot 接真实行情；UI 叙事"溜溜梅 LLM" | 多标的、用户自定义标的 |
...
| 费用 | ❓ Q7：无手续费（方案 2）或 IRMarket 包裹扣 1%（方案 1） | 动态费率 |
...
4. ❓ 手续费：Q7 待拍板（demo 无手续费 or 包裹扣费）
5. ✅ 报价节奏：可配间隔，3min 内完成全链路（D-05）
6. ✅ UI 链路：faucet→开仓(veto)→持仓→反向平仓→到期估值，无断点
7. ✅ 设计风格：卡片式、黄色主色
8. ⚠️ 标的：真实 06658.HK 行情源接入方式待开发时确认（bot 拉数源）
9. ❓ 其他遗漏场景（请在下方反馈）
...
| B3 | **手续费无处扣**：直接调 Monoracle 无扣费位；报价价差不可持续（会被否决套利吃掉） | **中** | ❓ → §十 Q7 |
...
- **方案 1（若启用手续费）**：交易经 IRMarket 包裹层入口，进 veto 前按名义额显式扣 1% 给做市商（符合 Q3=A）
- **方案 2（demo 最简，推荐）**：Demo 无手续费，bot 纯对手方演示
- **方案 3**：UI 层记账、事后手动结算（最不推荐，破坏"当场换手"体验）

### ❓ Q8（低）demo 叙事与真实标的

- **A**：UI 继续用"溜溜梅 LLM"叙事 + 真实 06658.HK 行情（代码真实、名字虚构）
- **B**：UI 改用 06658.HK 真实名称（锅圈食品），去掉 LLM 梗
- **C**：换一个真实代码且带"LLM"梗的标的（如有）
