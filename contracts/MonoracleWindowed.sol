...
/**
 * @title MonoracleWindowed
 * @notice IRMarket fork of the Monoracle veto-arbitrage price oracle.
 * @dev    Derived from upstream `github.com/dixia/monoracle` (contracts/Monoracle.sol, MIT).
 *         Single behavioral change: the verification window is no longer a fixed
 *         2-slot constant — each quote carries its own `expiryBlock` and stays
 *         vetoable until that block (inclusive). For IRMarket the window is aligned
 *         with the option round expiry (PRD D-13): the veto-arb "price challenge
...
contract MonoracleWindowed is ReentrancyGuard {
    ...
    struct Quote {
        ...
        uint256 expiryBlock;    // vetoable until this block (inclusive); after it, settleable
    ...
    /// @dev Vetoable while block.number <= quote.expiryBlock (window = option round).
    modifier inVerificationWindow(uint256 quoteId) {
        Quote storage q = quotes[quoteId];
        if (block.number > q.expiryBlock) {
            revert VerificationWindowExpired();
        }
        _;
    }

    /// @dev Settleable only after the quote's expiry block.
     ...
     * @param  baseToken    Address of base asset ERC20 (e.g. LLM)
     * @param  quoteToken   Address of quote asset ERC20 (e.g. HKD)
     ...
     * @param  expiryBlock  Vetoable until this block (inclusive); aligned with the
     *                      option round expiry. Must be > block.number.
    ...
    function submitQuote(
        ...
        emit QuoteSubmitted(quoteId, msg.sender, baseToken, quoteToken, baseAmount, quoteAmount, price, uint32(block.number), expiryBlock);
    }
     ...
     *         Available any time before the quote's expiryBlock (the LONG trade).
     ...
     *         Available any time before the quote's expiryBlock (the SHORT trade).
     * @param  quoteId ID of the active quote
     */
    ...
    /**
     * @notice Settle a quote that survived until its expiry block.
     *         Updates the canonical price feed for the asset pair.
     *         Settle order matters for the canonical price: settle older quotes
     *         first and the round's final quote LAST (it becomes the mark).
     * @param  quoteId ID of the active quote
     */
